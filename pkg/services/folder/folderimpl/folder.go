...
func (s *Service) DBMigration(db db.DB) {
	mg := migrator.NewMigrator(db.GetDBEngine(), s.cfg)
	...
	err := mg.Start(s.cfg.IsFeatureToggleEnabled(featuremgmt.FlagMigrationLocking), ss.dbCfg.MigrationLockAttemptTimeout)
	if err != nil {
		s.log.Error("the db migration on folder failed for : ", err.Error())
	}
}
